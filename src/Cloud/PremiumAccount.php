...
class PremiumAccount extends \HubletoMain\CoreClass
{

  public function getAccountUid()
  {
    $accountUid = $this->main->config->getAsString('cloud/accountUid');
    if (empty($accountUid)) {
      $accountUid = \Hubleto\Framework\Helper::generateUuidV4();
      $this->main->config->save('cloud/accountUid', $accountUid);
    }
    return $accountUid;
  }
  ...
  {
    $paymentVariableSymbol = $this->main->config->getAsString('cloud/paymentVariableSymbol');
    if (empty($paymentVariableSymbol)) {
      ...
      $this->main->config->save('cloud/paymentVariableSymbol', $paymentVariableSymbol);
    }
  ...
  public function getFreeTrialInfo(): array
  {
    ...
    $premiumAccountSince = $this->main->config->getAsString('cloud/premiumAccountSince');
    $freeTrialPeriodUntil = $this->main->config->getAsString('cloud/freeTrialPeriodUntil');
    $isTrialPeriod = $this->main->config->getAsBool('cloud/isTrialPeriod');
  ...
  {
    $subscriptionRenewalActive = $this->main->config->getAsBool('cloud/subscriptionRenewalActive');
    $subscriptionActiveUntil = $this->main->config->getAsString('cloud/subscriptionActiveUntil');
  ...
  public function recalculateCredit(): float
  {
    ...
    if (is_array($lastCreditData) && $lastCreditData['credit'] > 0 && $currentCredit <= 0) {
      $this->main->config->save('cloud/creditExhaustedOn', date('Y-m-d'));
    }
  ...
  {
    $this->main->config->save('cloud/premiumAccountSince', date('Y-m-d H:i:s'));
    $this->main->config->save('cloud/isTrialPeriod', '1');
    $this->main->config->save('cloud/freeTrialPeriodUntil', date('Y-m-d H:i:s', strtotime('+1 month')));
  ...
  {
    $this->main->config->save('cloud/subscriptionRenewalActive', '1');
    $this->extendSubscriptionByOneMonth();
  }

  public function deactivateSubscriptionRenewal(): void
  {
    $this->main->config->save('cloud/subscriptionRenewalActive', '0');
  }

  public function extendSubscriptionByOneMonth()
  {
    $this->main->config->save('cloud/subscriptionActiveUntil', date('Y-m-d H:i:s', strtotime('+1 month')));
  }

  public function premiumAccountActivated(): bool
  {
    $activated = !empty($this->main->config->getAsString('cloud/premiumAccountSince'));
    if (!$activated) {
